...
async function loadDesaInfo() {
  ...
  if (document.getElementById('agendaIndex')) {
    ...
    const agendaData = (beritaRes.data || []).filter(b => b.kategori === 'Agenda');
        ...
        '<div class="admin-card" style="padding:24px;border-radius:var(--radius-lg);box-shadow:var(--shadow-sm);border:1px solid var(--border)">' +
          '<div style="display:flex;align-items:center;gap:16px;margin-bottom:16px">' +
            '<div style="width:60px;height:60px;border-radius:16px;background:var(--primary-pale);display:flex;flex-direction:column;align-items:center;justify-content:center;color:var(--primary)">' +
              '<span style="font-size:1.5rem;font-weight:800;line-height:1">' + (a.tanggal ? new Date(a.tanggal).getDate() : "-") + '</span>' +
              '<span style="font-size:0.75rem;font-weight:600;text-transform:uppercase">' + (a.tanggal ? new Date(a.tanggal).toLocaleString("id-ID", {month:"short"}) : "") + '</span>' +
            '</div>' +
            '<div>' +
              '<h3 style="font-size:1.1rem;font-weight:700;color:var(--text-dark);margin:0 0 4px">' + (a.judul||"") + '</h3>' +
              '<span class="badge bg-emerald-100 text-emerald-800" style="font-size:0.7rem">Akan Datang</span>' +
            '</div>' +
          '</div>' +
          '<div style="font-size:0.85rem;color:var(--text-muted);margin-bottom:12px;line-height:1.5">' + (a.ringkasan||"") + '</div>' +
